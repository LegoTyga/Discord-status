...
smithbot.once('ready', () => {
    ...
    setInterval(() => {
        smithmta.query({
            ...
            smithbot.user.setActivity(`${state.raw.numplayers}/${state.maxplayers}`);
        }).catch(err => {
            console.log(err);
        });
    }, 5000);
...
smithbot.on('interactionCreate', async smithmsg => {
	...
	if (commandName === 'server') {
		smithmta.query({
            ...
            .addField(`Bot Developer :`,` - ! ˡᵉᵍᵒᶜʰᵃˢᵉ`,true)
            .addField(`Online Players :`,` - ${state.raw.numplayers}/${state.maxplayers}:person_frowning:`,true)
